"""Evaluate ANYmal-C multi-seed checkpoints after training completes."""
...
def find_latest_run(task_name, seed):
    """Find the latest run directory for a given task and seed."""
    log_dir = f"/home/hurricane/RL/CSGLoco/legged_gym/logs/{task_name}"
    ...
    runs.sort(key=lambda x: os.path.getmtime(os.path.join(log_dir, x)), reverse=True)

    # Find runs that match the seed pattern
    for run in runs:
        # Check if this run has the seed in its name or was created during our training
        return run  # Return the most recent
...
def evaluate(task_name, load_run, checkpoint_iter=1500, num_eval_steps=10000, num_envs=128,
...
def run_anymal_multiseed_eval():
    """Evaluate all ANYmal-C multi-seed checkpoints."""
    print("=" * 70)
    print("ANYmal-C Multi-Seed Evaluation")
    print("=" * 70)

    # Discover all available ANYmal runs
    tasks = {
        "safe_recovery_anymal": "anymal_vanilla",
        "safe_recovery_anymal_cat": "anymal_cat",
        "safe_recovery_anymal_fallen": "anymal_recovery",
    }
    ...
    for task, method_name in tasks.items():
        ...
        seed_id = 1
        for run in runs[-6:]:  # Last 6 runs
            print(f"  Evaluating {run} (as seed {seed_id})...")
            try:
                checkpoint = 2000 if "fallen" in task else 1500
                r = evaluate(task, run, checkpoint_iter=checkpoint, num_eval_steps=10000, num_envs=128,
                           force_range=[80, 150])
    ...
    print("\n" + "=" * 70)
    print("ANYmal-C Multi-Seed Evaluation Complete")
    print("=" * 70)

if __name__ == "__main__":
    print("Started: %s" % datetime.now().isoformat())
    run_anymal_multiseed_eval()
