"""Exp 4: A1 Rough Terrain Evaluation - Conservative Fix"""
...
def evaluate_rough(task_name, load_run, num_eval_steps=10000, num_envs=128,
    ...
    """Evaluate on rough terrain with proper initialization."""
    ...
    # Rough terrain settings - use heightfield with randomized terrain (not selected)
    env_cfg.terrain.mesh_type = "heightfield"
    env_cfg.terrain.measure_heights = True
    env_cfg.terrain.curriculum = False
    env_cfg.terrain.selected = False  # Use randomized instead of selected
    env_cfg.terrain.terrain_kwargs = None  # Not used with randomized

    # Rough terrain proportions: [smooth slope, rough slope, stairs up, stairs down, discrete]
    # Increase rough terrain proportion
    env_cfg.terrain.terrain_proportions = [0.1, 0.4, 0.2, 0.15, 0.15]

    # Ensure proper terrain dimensions
    env_cfg.terrain.terrain_length = 8.
    env_cfg.terrain.terrain_width = 8.
    env_cfg.terrain.num_rows = 10
    env_cfg.terrain.num_cols = 20
    env_cfg.terrain.horizontal_scale = 0.1
    env_cfg.terrain.vertical_scale = 0.005
    ...
    print("  Creating rough terrain env (heightfield, randomized)...")
    ...
    summary["meta/terrain_type"] = "rough_randomized"
...
def run_rough_eval():
    print("=" * 70)
    print("EXP 4: A1 Rough Terrain Evaluation (Seeds 1-3) - CONSERVATIVE FIX")
    print("=" * 70)
